...
論文を書いていると同じ変数が何度も出てくることがあります．
例えば，摩擦 Reynolds 数を $Ret$ と表す場合，毎回 `"Re"_tau` と書くのは面倒です．
そこで，このテンプレートでは `#let Ret = $"Re"_tau$` としておくことで，$Ret$ と簡単に出力できるようにしています．
これらの自作コマンドの定義は `settings.typ` の末尾にまとめてあります．
他にも自分がよく使う変数などを自作コマンドとして定義しておくと便利です．
